...
router.get('/status', async (req, res) => {
  try {
    ...
    // 获取最近的错误日志
    const recentErrors = await SystemLogService.getLogs({
      level: 'error',
      page: 1,
      pageSize: 10,
      sortBy: 'createdAt',
    ...
    // 获取系统启动时间（通过查找最近的 startup 日志）
    const startupLogs = await SystemLogService.getLogs({
      module: 'system',
      action: 'startup',
      page: 1,
      pageSize: 1,
    ...
    res.json({
      ...
      data: {
        stats: stats.data || [],
        recentErrors: recentErrors.data?.logs || [],
        systemStartTime,
        uptime: systemStartTime ? Date.now() - new Date(systemStartTime).getTime() : null
      }
    });
  } catch (error) {
    res.status(500).json({
