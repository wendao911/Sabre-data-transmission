  ...
  async connect(params = {}) {
    ...
    for (let attempt = 1; attempt <= 3; attempt += 1) {
      try {
        ...
        await SystemLogService.logSFTPStatus('connect', 'SFTP 连接成功', { host: cfg.host, port: cfg.port, user: cfg.username, attempt });
  ...
  async disconnect() {
    try {
      if (this.isConnected) {
        ...
        this.isConnected = false;
        this.lastConnectionTime = null;
        await SystemLogService.logSFTPStatus('disconnect', 'SFTP 连接已断开', { reason: 'manual_disconnect' });
      }
      return { success: true, message: 'SFTP连接已断开' };
    } catch (error) {
      await SystemLogService.logSFTPStatus('disconnect_failed', 'SFTP 断开连接失败', { error: error.message });
